...
$schoolUpper = ucwords($row['schoolname']);
    ...
    <section class="keuze">
       <div class="omallesheen">
        <article>
            <?php
    echo ' <h2 class="titleschool">'. $schoolName .'</h2>';
    ...
    while($row = $result4 ->fetch_assoc()){
      ...
      ?>      
       ...
       <p class="contact">Bekijk op de kaart</p>
        <?php
       $sql9 = "SELECT * FROM schools WHERE id=" . $id;
$result9 = $conn->query($sql9);


while($row = $result9 ->fetch_assoc()){
        echo '<a target="_blank" href="https://www.google.nl/maps/place/' . $row['adress'] .'+' . $row['street_number'] .',+' . $row['zipcode'] . '+Amsterdam/@"><img class="iconafb" src="Stylesheet/Afbeeldingen/iconlocatie.png"></a>';
        }        
        ?>
    </div>
